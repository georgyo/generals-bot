...
class Map(object):
    def __init__(self, start_data, data):
        ...
        self.replay_url = (
            _REPLAY_URLS["na"] + start_data["replay_id"]
        )  # String Replay URL # TODO: Use Client Region
        ...
        self.grid = [
            [Tile(x, y) for x in range(self.cols)] for y in range(self.rows)
        ...
        self.updateTurnGrid = [
            [int for x in range(self.cols)] for y in range(self.rows)
        ]  # 2D List of Tile Objects
    ...
    def updatePlayerInformation(self):
        ...
        for player in self.players:
            ...
            player.standingArmy = (
                self.scores[player.index]["total"] - self.scores[player.index]["tiles"]
            )
        ...
        for i in range(len(self.scoreHistory) - 2, 0, -1):
            ...
            if last != None:
                for j, player in enumerate(self.players):
                    ...
                    if (
                        abs(tileDelta) <= 2 and turn % 50 != 0
                    ):  # ignore army bonus turns and other player captures
        ...
        for i, player in enumerate(self.players):
            if not player.dead:
                if earliest != None:
                    ...
                    player.delta25tiles = (
                        self.players[i].tileCount - earliest[i]["tiles"]
                    )
    ...
    def do_actual_good_city_calculation(self):
        ...
        if expectArmyBonus or not expectCityBonus:
            logging.info(
                "do nothing, we can't calculate cities on a non-even turn, or on army bonus turns!"
            )
            return
            # if player.cityCount < cityCounts[i]:
            # 	player.cityCount = cityCounts[i]
        ...
        if lastScores == None:
            ...
            return
        logging.info(
            "myPlayer score {}, lastScores myPlayer total {}".format(
                myPlayer.score, lastScores[myPlayer.index]["total"]
            )
        )
        actualPlayerDelta = myPlayer.score - lastScores[myPlayer.index]["total"]
        logging.info(
            "otherPlayer score {}, lastScores otherPlayer total {}".format(
                otherPlayer.score, lastScores[otherPlayer.index]["total"]
            )
        )
        ...
        if realEnemyCities <= -40:
            ...
            logging.info(
                "set otherPlayer cityCount += 1 to {} because it appears he just took a city."
            )
    ...
    def handle_player_capture(self, text):
        ...
        print(
            "\n\n    ~~~~~~~~~\nPlayer captured: {} ({}) by {} ({})\n    ~~~~~~~~~\n".format(
                capturee, captureeIdx, capturer, capturerIdx
            )
        )
            ...
            for x in range(self.cols):
                for y in range(self.rows):
                    ...
                    if tile.player == captureeIdx:
                        ...
                        tile.update(
                            self, tile.tile, tile.army // 2, overridePlayer=capturerIdx
                        )
    ...
    def update(self, data):
        for player in self.players:
            if player != None:
                ...
                player.tiles = []
        if (
            self.complete and self.result == False
        ):  # Game Over - Ignore Empty Board Updates
            return self
        ...
        for x in range(self.cols):  # Update Each Tile
            for y in range(self.rows):
                ...
                armyMovedGrid[y][x] = curTile.update(
                    self, tile_type, army_count, isCity, isGeneral
                )
        ...
        for x in range(self.cols):
            for y in range(self.rows):
                ...
                if (
                    not curTile.visible
                    and (curTile.isCity or curTile.isGeneral)
                    and curTile.player >= 0
                    and self.turn % 2 == 0
                ):
                    curTile.army += 1
        ...
        self.players[self.player_index].cityCount = (
            len(self.players[self.player_index].cities) + 1
        )
    ...
    def _applyUpdateDiff(self, data):
        ...
        self._tile_grid = [
            [
                self._map_private[2 + self.cols * self.rows + y * self.cols + x]
                for x in range(self.cols)
            ]
            for y in range(self.rows)
        ]
        ...
        self._army_grid = [
            [self._map_private[2 + y * self.cols + x] for x in range(self.cols)]
            for y in range(self.rows)
        ]

        # Update Visible Cities
        self._visible_cities = [
            (c // self.cols, c % self.cols) for c in self._cities_private
        ]  # returns [(y,x)]

        # Update Visible Generals
        self._visible_generals = [
            (-1, -1) if g == -1 else (g // self.cols, g % self.cols)
...
def evaluateDualVisibleTileDiffs(tile, candidateTile):
    if (
        tile.delta.oldOwner == tile.delta.newOwner
...
def evaluateMoveFromFog(tile, candidateTile):
    ...
    if candidateDelta >= 0 and candidateDelta <= 2:
        ...
        logging.info(
            " (evaluateMoveFromFog) candidateTile {} army to {}".format(
                candidateTile.toString(), candidateTile.army
            )
        )
        return 100
...
def evaluateIslandFogMove(tile, candidateTile):
    ...
    if (
        candidateTile.visible
        and tile.army + candidateTile.delta.armyDelta < -1
        and candidateTile.player != -1
    ):
        ...
        candidateTile.army = 1
        logging.info(
            " (islandFog 1) tile {} army to {}".format(tile.toString(), tile.army)
        )
        logging.info(
            " (islandFog 1) candTile {} army to 1".format(candidateTile.toString())
        )
        return 50
    if tile.army - candidateTile.army < -1 and candidateTile.player != -1:
        ...
        candidateTile.army = 1
        logging.info(
            " (islandFog 2) tile {} army to {}".format(tile.toString(), tile.army)
        )
        logging.info(
            " (islandFog 2) candTile {} army to 1".format(candidateTile.toString())
        )
        return 30
    return -100
...
class Tile(object):
    ...
    def update(
        self, map, tile, army, isCity=False, isGeneral=False, overridePlayer=None
    ):
        ...
        if (
            (army == 0 and self.visible)
            or army > 0
            and (self.army != army or self.delta.oldOwner != self.delta.newOwner)
        ):  # Remember Discovered Armies
