...
class TerritoryClassifier:
    ...
    def scan(self):
        ...
        counts = new_map_matrix(
            self.map, lambda x, y: [0 for n in range(len(self.map.players) + 1)]
        )
        startTime = time.time()
        ...
        def foreach_near_updated_tiles(evaluatingTile):
            def countFunc(tile):
                ...
                if not evaluatingTile.discovered:
                    ...
                    if currentTerritory != -1:
                        ...
                        if (
                            tile.discovered
                            and not evaluatingTile.discovered
                            and currentTerritory != self.map.player_index
                        ):
                            counts[evaluatingTile.x][evaluatingTile.y][
                                currentTerritory
                            ] += 0.3
            ...
            if evaluatingTile.player != maxPlayer and evaluatingTile.player != -1:
                logging.info(
                    "Tile {} is in player {} {} territory".format(
                        evaluatingTile.toString(), maxPlayer, userName
                    )
                )
        ...
        logging.info(
            "  Scanning territory around {}".format(
                " - ".join([tile.toString() for tile in startTiles])
            )
        )
        breadth_first_foreach(
            self.map,
            startTiles,
            undiscoveredCounterDepth,
            foreach_near_updated_tiles,
            None,
            lambda tile: tile.mountain,
            None,
