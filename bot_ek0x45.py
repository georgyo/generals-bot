...
class EklipZBot(object):
	...
	def in_gather_split(self, turnOffset = 0):
		return (self._map.turn + turnOffset + self.timings.offsetTurns) % self.timings.cycleTurns < self.timings.splitTurns

	def in_expand_split(self, turnOffset = 0):
		return (self._map.turn + turnOffset + self.timings.offsetTurns) % self.timings.cycleTurns >= self.timings.splitTurns
	...
	def get_timings(self):
		...
		longSpawns = self.target_player_gather_path != None and self.distance_from_general(self.targetPlayerExpectedGeneralLocation) > 23
	...
	def timing_gather(self, startTiles, negativeTiles = None, skipTiles = None, force = False):
		turnOffset = self._map.turn + self.timings.offsetTurns
		turnCycleOffset = turnOffset % self.timings.cycleTurns
		if force or (self._map.turn >= 50 and turnCycleOffset < self.timings.splitTurns and startTiles != None and len(startTiles) > 0):
			...
			if depth > 30:
				...
				gatherMove = self.get_gather_move(treeNodes, None, minGatherAmount = 1, preferNeutral = False, allowNonKill = True)				
				...
				enemyDistanceMap = build_distance_map(self._map, [self.targetPlayerExpectedGeneralLocation])
				self.gatherNodes = greedy_backpack_gather(self._map, startTiles, depth, negativeTiles = negativeTiles, searchingPlayer = self.general.player, skipFunc = skipFunc, viewInfo = self.viewInfo, skipTiles = skipTiles, distPriorityMap = enemyDistanceMap)
	...
	def select_move(self, allowRetry = True):
		...
		if (len(self.largeTilesNearEnemyKings.keys()) > 0):
			for enemyGeneral in self._map.generals:
				...
				for depth in range (2, 6):
					...
					killPath = dest_breadth_first_target(self._map, [enemyGeneral], max(targetArmy, 1), 0.05, depth, attackNegTiles, self.general.player, False, 4, noLog = True)
				...
				depth = max(7, int(self.distance_from_general(self.targetPlayerExpectedGeneralLocation) // 3))
				...
				killPath = dest_breadth_first_target(self._map, [enemyGeneral], targetArmy, 0.05, depth, attackNegTiles, self.general.player, False, 4, noLog = True)
		...
		if threat != None and not self.allIn and (turnsTillDanger > -1 and threat.threatType == ThreatType.Kill):
			...
			if (threat != None and threat.threatType == ThreatType.Kill):
				...
				if (path != None and path.length > 0):
					...
					move = self.gather_to_target_tiles(defenseTiles, 0.2, threat.turns, None, defenseNegatives, threat.threatValue, useTrueValueGathered = True)
					if move:
						self.curPath = None
					...
					killPath = dest_breadth_first_target(self._map, [self.targetPlayerExpectedGeneralLocation], 10, 0.1, threat.turns - 1, None, dontEvacCities=False)
					if killPath != None:
		...
		if self.targetingArmy and not self.targetingArmy.scrapped and self.targetingArmy.tile.army > 2:
			if self.distance_from_general(self.targetingArmy.tile) < self.distance_from_opp(self.targetingArmy.tile) and self.should_kill(self.targetingArmy.tile):
				...
				if path:
					...
					if not self.detect_repetition(move, 6, 3):						
						...
						movingAwayFromEnemy = self.board_analysis.intergeneral_analysis.bMap[path.start.tile.x][path.start.tile.y] < self.board_analysis.intergeneral_analysis.bMap[path.start.next.tile.x][path.start.next.tile.y]
						move.move_half = path.tail.tile.army < path.value / 2 and movingAwayFromEnemy
						self.info("Continuing to kill army {} with half {} path {}".format(self.targetingArmy.toString(), move.move_half, path.toString()))
						self.viewInfo.paths.appendleft(PathColorer(path, 158, 158, 158, 255, 10, 200))
						return move
					else:
		...
		if not self.allIn and (len(paths) == 0):
			logging.info("Checking for city captures ({:.3f} in)".format(time.time() - start))
		...
		if self.finishingExploration or self.allIn or demolishingTargetPlayer:
			logging.info("Checking if we should finish exploration ({:.3f} in)".format(time.time() - start))
		...
		if len(paths) == 0 and threat != None and threat.path.length < self.distance_from_general(self.targetPlayerExpectedGeneralLocation) // 2 + 1:
			...
			if threatKill:
				...
				if not self.detect_repetition(move, 6, 3):
					self.viewInfo.paths.appendleft(PathColorer(threatKill, 158, 158, 158, 255, 10, 200))
					threatKill.calculate_value(self.general.player)
					movingAwayFromEnemy = self.board_analysis.intergeneral_analysis.bMap[saveTile.x][saveTile.y] < self.board_analysis.intergeneral_analysis.bMap[nextTile.x][nextTile.y]
					move.move_half = threatKill.tail.tile.army < threatKill.value / 2 and movingAwayFromEnemy
					self.info("Threat kill. away {}, half {}, {}".format(movingAwayFromEnemy, move.move_half, threatKill.toString()))
					return move
		...
		if (len(paths) == 0 and threat != None and threat.threatType == ThreatType.Vision 
			...
			if path:
				...
				self.viewInfo.paths.appendleft(PathColorer(path, 0, 0, 255, 255, 10, 200))
				return Move(path.start.tile, path.start.next.tile)
			else:
				self.targetingArmy = None
		...
		if len(paths) == 0 and (self.curPath == None or self.curPath.start.next == None) and not self.defendEconomy:
			...
			logging.info("------  ATTACK LAUNCH ------- ({:.3f} in)".format(time.time() - start))
			# reduce the length of the path to allow for other use of the army

			targetPathLength = path.length * 6 / 9 + 1
			if self.allIn:
				targetPathLength = path.length * 4 / 7 + 1
			...
			tempTimingOffset = 0
			if timingTurn - tempTimingOffset >= self.timings.splitTurns:
				pathWorth = get_player_army_amount_on_path(self.target_player_gather_path, self.general.player)
				expStartTime = time.time()
				inAttackWindow = timingTurn < self.timings.splitTurns + origPathLength / 3
				self.info("  Inside +split if. path.value {}, timingTurn {} < self.timings.splitTurns + origPathLength {} / 3 {:.1f}: {}".format(path.value, timingTurn, origPathLength, self.timings.splitTurns + origPathLength / 3, inAttackWindow))
				minArmy = math.sqrt(player.standingArmy) * 4
				...
				if len(paths) == 0:
					...
					path = self.explore_target_player_undiscovered(undiscNeg)
					if (path != None):
						logging.info("found depth {} dest bfs kill on target undisc\n{}".format(path.length, path.toString()))
						paths.append(path)
			
				if len(paths) == 0 and not self.all_in_counter > 50 and not self.defendEconomy:
					# no timing gather move, optimal expand?
					path = self.get_optimal_expansion(self.timings.cycleTurns - (self._map.turn + self.timings.offsetTurns) % self.timings.cycleTurns, negativeTiles)
					if path:
						move = Move(path.start.tile, path.start.next.tile)
		...
		if not self.allIn and len(paths) == 0 and (self.curPath == None or self.curPath.start.next == None):
			...
			if self._map.turn % self.timings.cycleTurns < self.timings.cycleTurns / 14 and not self.winning_on_economy(1.25, 0, self.targetPlayer, -3):
				...
				useLeafMove = True
			if useLeafMove:
				logging.info("----- Leaf moves??? We've really come full circle -----")
				moves = self.prioritize_expansion_leaves(self.leafMoves)
				if len(moves) > 0:
					move = moves[0]
					self.info(" oh wow, doing leafMove {}".format(move.toString()))
					return move


		if len(paths) == 0 and (self.curPath == None or self.curPath.start.next == None):
			logging.info("Checking for primary gather phase ({:.3f} in)".format(time.time() - start))
			tryGather = True
			player = self._map.players[self.general.player]
			modVal = 0
			enemyGather = False
			...
			if tryGather:
				...
				if not self.allIn and not self.defendEconomy:
					# Add support for 'green arrows', pushing outer territory towards enemy territory.
					goodLeaves = self.board_analysis.find_flank_leaves(self.leafMoves, minAltPathCount = 2, maxAltLength = shortestLength + 4)
				...
				if (enemyGather or neutralGather) and not self.allIn:
					...
					move = self.timing_gather(leafGatherTargets, gatherNegatives, skipTiles = None, force = True)
					if (move != None):
						self.curPath = None
				...
				move = self.timing_gather(gatherTargets, gatherNegatives, skipTiles = None, force = True)
				if (move != None):
					self.curPath = None
						...
						if (self.curPath.start.tile in negativeTiles and self.curPath.start.tile.army > 5):
							...
							if threat != None:
								...
								if killThreatPath != None:
									...
									self.viewInfo.paths.appendleft(PathColorer(killThreatPath, 158, 158, 158, 255, 10, 200))
									self.targetingArmy = self.armyTracker.armies[threat.path.start.tile]
	...
	def kill_army(self, army, allowGeneral = False):
		...
		if path:
			self.viewInfo.paths.append(PathColorer(path.clone(), 0,0,0, 200, 5, 100))
			return self.kill_enemy_path(path, allowGeneral)
		else:
	...
	def kill_enemy_path(self, threatPath, allowGeneral = False):
		...
		# First try one move kills on next tile, since I think this is broken in the loop for whatever reason...
		for adj in threatPath.start.next.tile.moveable:
			if adj.player == self.general.player and adj.army > threatValue:
				path = Path()
				path.add_next(adj)
	...
	def get_optimal_exploration(self, turns, negativeTiles = None, valueFunc = None, priorityFunc = None, initFunc = None, skipFunc = None, allowLeafMoves = True, calculateTrimmable = True):
		...
		ourArmies = where(self.armyTracker.armies.values(), lambda army: army.player == self.general.player and army.tile.player == self.general.player and army.tile.army > 2)
		...
		if not skipFunc:
			def skip_after_out_of_army(nextTile, nextVal):
				wastedMoves, pathPriorityDivided, negArmyRemaining, negRevealedCount, enemyTiles, neutralTiles, pathPriority, distSoFar, tileSetSoFar, revealedSoFar = nextVal
				if negArmyRemaining >= 0:
					return True
				if wastedMoves > 6:
					return True
		...
		if not valueFunc:
			def value_priority_army_dist(currentTile, priorityObject):
				wastedMoves, pathPriorityDivided, negArmyRemaining, negRevealedCount, enemyTiles, neutralTiles, pathPriority, distSoFar, tileSetSoFar, revealedSoFar = priorityObject
				# negative these back to positive
				posPathPrio = 0-pathPriorityDivided
				dist = distSoFar + 2
				value = 0-(negRevealedCount + enemyTiles * 2 + neutralTiles + pathPriority) / dist
				if negArmyRemaining >= 0:
					value = -1000
		...
		if not priorityFunc:
			def default_priority_func(nextTile, currentPriorityObject):
				wastedMoves, pathPriorityDivided, negArmyRemaining, negRevealedCount, enemyTiles, neutralTiles, pathPriority, distSoFar, tileSetSoFar, revealedSoFar = currentPriorityObject
				armyRemaining = 0 - negArmyRemaining
				nextTileSet = tileSetSoFar.copy()
				distSoFar += 1
				# weight tiles closer to the target player higher
				...
				#if generalPlayer.tileCount < 46:
				#	logging.info("nextTile {}, newPathPriority / distSoFar {:.2f}, armyRemaining {}, newPathPriority {}, distSoFar {}, len(nextTileSet) {}".format(nextTile.toString(), newPathPriority / distSoFar, armyRemaining, newPathPriority, distSoFar, len(nextTileSet)))
				return (wastedMoves, newPathPriority / distSoFar, 0-armyRemaining, negRevealedCount, enemyTiles, neutralTiles, newPathPriority, distSoFar, nextTileSet, nextRevealedSet)
		...
		if not initFunc:
			def initial_value_func_default(tile):
				...
				for adj in tile.adjacents:
					startingAdjSet.add(adj)
				return (0, 10, 0-tile.army, 0, 0, 0, 0, 0, startingSet, startingAdjSet)
	...
	def explore_target_player_undiscovered(self, negativeTiles, targetArmy = 1):
		...
		if not self.allIn and self.explored_this_turn:
			...
			turns = self.timings.cycleTurns - (self._map.turn + self.timings.offsetTurns) % self.timings.cycleTurns
			if turns < 6:
				turns = 6
			if self.allIn:
				turns = 12
		...
		if self.allIn:
			...
			if path != None:
				path = self.get_value_per_turn_subsegment(path, 0.9)
				if path.length > 2:
					path = path.get_subsegment(2)
			return path

		## don't explore to 1 army from inside our own territory
		##if not self.in_gather_split():
		#if negativeTiles == None:
		#	negativeTiles = set()
		#negativeTiles = negativeTiles.copy()
		#for tile in genPlayer.tiles:
	...
	def get_threat_killer_move(self, threat, searchTurns, negativeTiles):
		...
		if largestTile != None:
			if threat.threatValue - largestTile.army + threatModifier < 0:
				...
				bestPath = dest_breadth_first_target(self._map, dict, armyAmount + threatModifier - largestTile.army - 1, 0.1, searchTurns - savePathSearchModifier, negativeTilesIncludingThreat, ignoreGoalArmy=True)
				if bestPath != None:
					self.viewInfo.paths.appendleft(PathColorer(bestPath, 250, 250, 250, 200, 12, 100))
					if (largestTile.army > 10 or threat.threatValue <= largestTile.army):
	...
	def capture_cities(self, targets, negativeTiles):
		...
		if self.targetPlayer == -1 or self._map.remainingPlayers == 2:
			if targetPlayer == None or (self.in_gather_split() and (player.cityCount < targetPlayer.cityCount or proactivelyTakeCity)):
	...
	def get_optimal_expansion(self, turns, negativeTiles = None, valueFunc = None, priorityFunc = None, initFunc = None, skipFunc = None, allowLeafMoves = True, calculateTrimmable = True):
		...
		if not skipFunc:
			def skip_after_out_of_army(nextTile, nextVal):
				wastedMoves, distSoFar, tileCapturePoints, negArmyRemaining, enemyTiles, neutralTiles, pathPriority, tileSetSoFar, adjacentSetSoFar, enemyExpansionValue, enemyExpansionTileSet = nextVal
		...
		if not valueFunc:
			def value_priority_army_dist(currentTile, priorityObject):
				wastedMoves, distSoFar, tileCapturePoints, negArmyRemaining, enemyTiles, neutralTiles, pathPriority, tileSetSoFar, adjacentSetSoFar, enemyExpansionValue, enemyExpansionTileSet = priorityObject
				# negative these back to positive
				#return (posPathPrio, 0-armyRemaining, distSoFar)
				value = -1000
				dist = 1
				if negArmyRemaining < 0 and distSoFar > 0 and tileCapturePoints < 0:
					...
					value = 0-tileCapturePoints - len(enemyExpansionTileSet)
				return (value / dist, 
							0-enemyTiles / dist, 
									value,
		...
		if not priorityFunc:
			def default_priority_func(nextTile, currentPriorityObject):
				wastedMoves, distSoFar, tileCapturePoints, negArmyRemaining, enemyTiles, neutralTiles, pathPriority, tileSetSoFar, adjacentSetSoFar, enemyExpansionValue, enemyExpansionTileSet  = currentPriorityObject
				armyRemaining = 0 - negArmyRemaining
				nextTileSet = tileSetSoFar.copy()
				distSoFar += 1
				# weight tiles closer to the target player higher
				addedPriority = -10 - max(2, distMap[nextTile.x][nextTile.y] / 3)
				...
				if nextTile not in nextTileSet:
					...
					if usefulMove:
						...
						#addedPriority += count(releventAdjacents, lambda adjTile: not adjTile.discovered) / 2
						## points for revealing tiles in the fog
						addedPriority += count(releventAdjacents, lambda adjTile: not adjTile.visible) / 2
				else:
					wastedMoves += 1
				iter[0] += 1
				nextAdjacentSet = adjacentSetSoFar.copy()
				...
				for adj in nextTile.moveable:
					if adj.army >= 3 and adj.player != searchingPlayer and adj.player != -1 and adj not in negativeTiles and adj not in tileSetSoFar:
						nextEnemyExpansionSet.add(adj)
						enemyExpansionValue += (adj.army - 1) // 2
						tileCapturePoints += 0.3  # 
				newPathPriority = pathPriority - addedPriority
				...
				return (wastedMoves, distSoFar, tileCapturePoints, 0-armyRemaining, enemyTiles, neutralTiles, newPathPriority, nextTileSet, nextAdjacentSet, enemyExpansionValue, nextEnemyExpansionSet)
			priorityFunc = default_priority_func
		...
		if not initFunc:
			def initial_value_func_default(tile):
				...
				for adj in tile.moveable:
					if adj.player != searchingPlayer and adj.player != -1 and adj not in negativeTiles:
						startingEnemyExpansionTiles.add(adj)
						enemyExpansionValue += (adj.army - 1) // 2
						tileCapturePoints += 0.5
				return (0, 0, tileCapturePoints, 0-tile.army, 0, 0, 0,  startingSet, startingAdjSet, enemyExpansionValue, startingEnemyExpansionTiles)
			initFunc = initial_value_func_default
			...
			if path:
				...
				while node != None:
					if node.tile not in negativeTiles and node.tile not in visited:
						if node.tile.player == self.targetPlayer or (not node.tile.visible and self.territories.territoryMap[node.tile.x][node.tile.y] == self.targetPlayer):
							# slight boost to taking opp tiles
							tilesGrabbed += 2.2
						elif node.tile.player == -1:
							tilesGrabbed += 1
		...
		if len(maxKnapsackedPaths) > 0:
			...
			if totalTrimmable > 0:
				...
				for i in range(trimRange):
					...
					otherValueWeightedByTrim = otherValue - i
			...
			for pathTuple in maxKnapsackedPaths:
				...
				thisVal = (0-friendlyCityCount, 0 - trimmableVal, 0 - enemyDistMap[curPath.start.tile.x][curPath.start.tile.y], tilesCaptured / (curPath.length))
				if thisVal > maxVal:
					maxVal = thisVal
					path = curPath
					logging.info("new max val, eval [{}], path {}".format('], ['.join(str(x) for x in maxVal), path.toString()))
				else:
					logging.info("NOT max val, eval [{}], path {}".format('], ['.join(str(x) for x in maxVal), path.toString()))
	...
	def get_path_to_target_player(self, skipEnemyCities = False, cutLength = 19):
		...
		path = self.get_path_to_target(maxTile, skipEnemyCities = skipEnemyCities, preferNeutral = self._map.remainingPlayers < 3, fromTile = fromTile)
		if path.length > cutLength:
			path = path.get_subsegment(cutLength, end = True)
	...
	def get_best_defense(self, tile, turns):
		...
		valuePerTurnPath = breadth_first_dynamic_max(self._map, startTilesDict, valueFunc, 0.1, turns, noNeutralCities=True,
							...
							preferNeutral = False, noLog = True)
		logging.info("Best defense: {}".format(valuePerTurnPath))
	...
	def scan_map(self):
		...
		for player in self._map.players:
			...
			if not player.dead and player.index != self._map.player_index and seenPlayer:
				...
				curScore += player.tileCount + player.cityCount * 20 - player.standingArmy ** 0.85
