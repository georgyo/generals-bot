...
def new_map_matrix(map, initialValueXYFunc):
    return [
        [initialValueXYFunc(x, y) for y in range(map.rows)] for x in range(map.cols)
    ]


def new_tile_matrix(map, initialValueTileFunc):
    return [
        [initialValueTileFunc(map.grid[y][x]) for y in range(map.rows)]
...
def dest_breadth_first_target(
    ...
    while not frontier.empty():
        ...
        if skipTiles != None and current in skipTiles:
            if not noLog and iter < 100:
                logging.info(
                    "PopSkipped skipTile current {}, army {}, goalInc {}, targetArmy {}".format(
                        current.toString(), army, goalInc, targetArmy
                    )
                )
            continue
        ...
        if (
            ...
            or (
                current.isCity
                and noNeutralCities
                and current.player == -1
                and current not in goalList
            )
            or (not current.discovered and current.isobstacle())
        ):
            if not noLog and iter < 100:
                logging.info(
                    "PopSkipped Mountain, neutCity or Obstacle current {}".format(
                        current.toString()
                    )
                )
            continue
        ...
        if nextArmy > targetArmy and nextArmy > 0 and nextArmy > foundArmy:
            ...
            if not noLog and iter < 100:
                logging.info(
                    "GOAL popped {}, army {}, goalInc {}, targetArmy {}, processing".format(
                        current.toString(), nextArmy, goalInc, targetArmy
                    )
                )
            break
        ...
        if not noLog and iter < 100:
            logging.info(
                "Popped current {}, army {}, goalInc {}, targetArmy {}, processing".format(
                    current.toString(), nextArmy, goalInc, targetArmy
                )
            )
        if newDist <= maxDepth and not foundGoal:
            for next in current.moveable:  # new spots to try
                frontier.put(
                    ((newDist, 0 - nextArmy), next, newDist, nextArmy, goalInc, current)
                )
    if not noLog:
        logging.info(
            "BFS DEST SEARCH ITERATIONS {}, DURATION: {:.3f}, DEPTH: {}, FOUNDDIST: {}".format(
                iter, time.time() - start, depthEvaluated, foundDist
            )
        )
    if foundDist < 0:
        return None
    ...
    logging.info(
        "DEST BFS FOUND KILLPATH OF LENGTH {} VALUE {}\n{}".format(
            pathObject.length, pathObject.value, pathObject.toString()
        )
    )
    return pathObject
...
def a_star_kill(
    ...
    if isinstance(startTiles, dict):
        for start in startTiles.keys():
            ...
            logging.info(
                "a* enqueued start tile {} with extraArmy {}".format(
                    start.toString(), requireExtraArmy
                )
            )
    ...
    while not frontier.empty():
        ...
        if dist < maxDepth:
            for next in current.moveable:  # new spots to try
                ...
                if restrictionEvalFuncs != None:
                    if current in restrictionEvalFuncs:
                        if not restrictionEvalFuncs[current](next):
                            logging.info(
                                "dangerous, vetod: {},{}".format(current.x, current.y)
                            )
                            continue
                        else:
                ...
                inc = (
                    0
                    if not ((next.isCity and next.player != -1) or next.isGeneral)
                    else (dist + 1) / 2
                )
                ...
                if (
                    nextArmy <= 0 and army > 0
                ):  # prune out paths that go negative after initially going positive
    ...
    logging.info(
        "A* KILL SEARCH ITERATIONS {}, DURATION: {:.3f}, DEPTH: {}".format(
            iter, time.time() - start, depthEvaluated
        )
    )
    ...
    logging.info(
        "A* FOUND KILLPATH OF LENGTH {} VALUE {}\n{}".format(
            pathObject.length, pathObject.value, pathObject.toString()
        )
    )
    ...
    if pathObject.value < requireExtraArmy:
        logging.info(
            "A* path {} wasn't good enough, returning none".format(
                pathObject.toString()
            )
        )
        return None
...
def breadth_first_dynamic(
    ...
    if isinstance(startTiles, dict):
        ...
        for tile in startTiles:
            ...
            if (
                not ignoreStartTile
                and tile.player != searchingPlayer
                and tile.player != -1
            ):
                negEnemyTileCount = -1
    ...
    while not frontier.empty():
        ...
        if (
            dist not in visited[current.x][current.y]
            or visited[current.x][current.y][dist][0] > prioVals
        ):
    ...
    logging.info(
        "BFS-DYNAMIC ITERATIONS {}, DURATION: {:.3f}, DEPTH: {}".format(
            iter, time.time() - start, depthEvaluated
        )
    )
    if foundDist >= 1000:
    ...
    logging.info(
        "DYNAMIC BFS FOUND PATH LENGTH {} VALUE {}\n   {}".format(
            pathObject.length, pathObject.value, pathObject.toString()
        )
    )
    return pathObject
...
def greedy_backpack_gather(
    ...
    logging.info(
        "Trying greedy-bfs-gather. Turns {}. Searching player {}".format(
    ...
    if baseCaseFunc == None:
        ...
        def default_base_case_func(tile, startingDist):
            ...
            if tile.player != searchingPlayer:
                logging.info(
                    "tile {} was not owned by searchingPlayer {}, adding its army {}".format(
                        tile.toString(), searchingPlayer, tile.army
                    )
                )
                startArmy = tile.army

            logging.info(
                "tile {} got base case startArmy {}, startingDist {}".format(
                    tile.toString(), startArmy, startingDist
                )
            )
    ...
    while valuePerTurnPath != None:
        if (
            valuePerTurnPath.tail.tile.army <= 1
            or valuePerTurnPath.tail.tile.player != searchingPlayer
        ):
            logging.info(
                "TERMINATING greedy-bfs-gather PATH BUILDING DUE TO TAIL TILE {} THAT WAS < 1 OR NOT OWNED BY US. PATH: {}".format(
                    valuePerTurnPath.tail.tile.toString(), valuePerTurnPath.toString()
                )
            )
            break
        logging.info(
            "Adding valuePerTurnPath (v/t {:.3f}): {}".format(
                (valuePerTurnPath.value - valuePerTurnPath.start.tile.army + 1)
                / valuePerTurnPath.length,
                valuePerTurnPath.toString(),
            )
        )
        # if viewInfo:
        ...
        while node != None:
            ...
            negativeTiles.add(node.tile)
            logging.info(
                "Including tile {},{} in startTilesDict at newDist {}  (distance {} addlDist {})".format(
                    node.tile.x, node.tile.y, newDist, distance, addlDist
                )
            )
            # if viewInfo:
        ...
        logging.info(
            "Searching for the next path with remainingTurns {}".format(remainingTurns)
        )
        valuePerTurnPath = breadth_first_dynamic_max(
            map,
            startTilesDict,
    ...
    logging.info(
        "Concluded greedy-bfs-gather built from {} path segments. Duration: {:.3f}".format(
            itr, time.time() - startTime
        )
    )
    rootNodes = list(
        where(treeNodeLookup.values(), lambda treeNode: treeNode.fromTile == None)
    )
...
def recalculate_tree_values(
    ...
    logging.info(
        "recalculate_tree_values {}, searchingPlayer {}, onlyCalculateFriendlyArmy {}".format(
            rootNode.tile.toString(), searchingPlayer, onlyCalculateFriendlyArmy
        )
    )
    sum = -1
    ...
    for child in rootNode.children:
        recalculate_tree_values(
            child, negativeTiles, searchingPlayer, onlyCalculateFriendlyArmy, viewInfo
        )
        sum += child.value
    if viewInfo:
...
def get_tree_move(gathers, priorityFunc, valueFunc):
    ...
    while q.qsize() > 0:
        ...
        if len(curGather.children) == 0:
            ...
            if curGather.fromTile != None and (
                highestValue == None or thisValue > highestValue
            ):
                highestValue = thisValue
                highestValueMove = Move(curGather.tile, curGather.fromTile)
                logging.info(
                    "new highestValueMove {}!".format(highestValueMove.toString())
                )
    ...
    if highestValueMove == None:
        return None
    logging.info(
        "highestValueMove in get_tree_move was {}!".format(highestValueMove.toString())
    )
...
def breadth_first_dynamic_max(
    ...
    while not frontier.empty():
        ...
        # TODO no globalVisitedSet
        # if current in globalVisitedSet or (skipTiles != None and current in skipTiles):
        if (useGlobalVisitedSet and current in globalVisitedSet) or (
            skipTiles != None and current in skipTiles
        ):
            continue
        ...
        if maxValue == None or newValue > maxValue:
            ...
            if logResultValues:
                if parent != None:
                    ...
                    parentString = "None"
                logging.info(
                    "+Tile {} from {} is new max value: [{}]  (dist {})".format(
                        current.toString(),
                        parentString,
                        "], [".join("{:.3f}".format(x) for x in newValue),
                        dist,
                    )
                )
            maxValue = newValue
    ...
    if not noLog:
        logging.info(
            "BFS-DYNAMIC-MAX ITERATIONS {}, DURATION: {:.3f}, DEPTH: {}".format(
                iter, time.time() - start, depthEvaluated
            )
        )
    if foundDist >= 1000:
    ...
    if pathObject.length == 0:
        if not noLog:
            logging.info(
                "BFS-DYNAMIC-MAX FOUND PATH LENGTH {} VALUE {}, returning NONE!\n   {}".format(
                    pathObject.length, pathObject.value, pathObject.toString()
                )
            )
        if includePathValue:
        ...
        if not noLog:
            logging.info(
                "BFS-DYNAMIC-MAX FOUND PATH LENGTH {} VALUE {}\n   {}".format(
                    pathObject.length, pathObject.value, pathObject.toString()
                )
            )
    if includePathValue:
...
def bidirectional_breadth_first_dynamic(
    ...
    logging.info(
        "BFS-FIND ITERATIONS {}, DURATION: {:.3f}, DEPTH: {}".format(
            iter, time.time() - start, depthEvaluated
        )
    )
    if foundDist >= 1000:
...
def breadth_first_find_queue(
    ...
    while len(frontier) > 0:
        ...
        if visited[current.x][current.y] != None or (
            skipTiles != None and current in skipTiles
        ):
            continue
        if goalFunc(current, army, dist) and (
            dist < foundDist or (dist == foundDist and army > foundArmy)
        ):
            foundGoal = True
            foundDist = dist
            foundArmy = army
        ...
        if dist <= maxDepth and not foundGoal:
            for next in current.moveable:  # new spots to try
                ...
                newDist = dist + 1
                if (
                    visited[next.x][next.y] == None
                    or visited[next.x][next.y][0] < nextArmy
                ):
                    visited[next.x][next.y] = (nextArmy, current)
                frontier.appendleft((next, newDist, nextArmy, goalInc))

    logging.info(
        "BFS-FIND-QUEUE ITERATIONS {}, DURATION: {:.3f}, DEPTH: {}".format(
            iter, time.time() - start, depthEvaluated
        )
    )
    if foundDist >= 1000:
    ...
    logging.info(
        "BFS-FIND-QUEUE found path OF LENGTH {} VALUE {}\n{}".format(
            pathObject.length, pathObject.value, pathObject.toString()
        )
    )
    return pathObject
...
def breadth_first_foreach(
    ...
    if not noLog:
        logging.info(
            "Completed breadth_first_foreach. startTiles[0] {},{}: ITERATIONS {}, DURATION {:.3f}, DEPTH {}".format(
                startTiles[0].x, startTiles[0].y, iter, time.time() - start, dist
            )
        )
...
def breadth_first_foreach_dist(
    ...
    if not noLog:
        logging.info(
            "Completed breadth_first_foreach_dist. startTiles[0] {},{}: ITERATIONS {}, DURATION {:.3f}, DEPTH {}".format(
                startTiles[0].x, startTiles[0].y, iter, time.time() - start, dist
            )
        )
...
def solve_knapsack(items, capacity, weights, values):
    ...
    for i in range(n, 0, -1):
        ...
        if i == 0:
            logging.info(
                "i == 0 in knapsack items determiner?? res {} i {} w {}".format(
                    res, i, w
                )
            )
            break
        if w < 0:
            logging.info(
                "w < 0 in knapsack items determiner?? res {} i {} w {}".format(
                    res, i, w
                )
            )
            break
        ...
        if res == K[i - 1][w]:
            ...
            logging.info(
                "item at index {} with value {} and weight {} was included... adding it to output. (Res {})".format(
                    i - 1, values[i - 1], weights[i - 1], res
                )
            )
            ...
            w = w - weights[i - 1]
    logging.info(
        "knapsack completed on {} items for capacity {} finding value {} in Duration {:.3f}".format(
            n, capacity, K[n][capacity], time.time() - timeStart
        )
    )
