...
class GeneralsViewer(object):
    ...
    def updateGrid(self, update):
        ...
        self._scores = sorted(
            update.scores, key=lambda general: general["total"], reverse=True
        )  # Sort Scores
    ...
    def get_line_arrow(self, r, g, b, width=3):
        s = pygame.Surface(
            (CELL_WIDTH + 2 * CELL_MARGIN, CELL_HEIGHT + 2 * CELL_MARGIN)
        )
        ...
        pygame.draw.line(
            s,
            (r, g, b),
            ((CELL_WIDTH) // 2 + CELL_MARGIN, 0),
            ((CELL_WIDTH) // 2 + CELL_MARGIN, CELL_HEIGHT),
            width,
        )
        pygame.draw.line(
            s,
            (r, g, b),
            ((CELL_WIDTH) // 2 + CELL_MARGIN, CELL_HEIGHT),
            (CELL_WIDTH * 2.5 / 8 + CELL_MARGIN, CELL_HEIGHT * 5 / 8),
            width,
        )
        pygame.draw.line(
            s,
            (r, g, b),
            ((CELL_WIDTH) // 2 + CELL_MARGIN, CELL_HEIGHT),
            (CELL_WIDTH * 5.5 / 8 + CELL_MARGIN, CELL_HEIGHT * 5 / 8),
            width,
        )
        return s
    ...
    def _initViewier(self, position):
        ...
        # Set Window Size
        window_height = (
            self._map.rows * (CELL_HEIGHT + CELL_MARGIN)
            + CELL_MARGIN
            + SCORES_ROW_HEIGHT
            + INFO_ROW_HEIGHT
        )
        ...
        self.logDirectory = "H:\\GeneralsLogs\\{}-{}".format(
            fileSafeUserName, self.repId
        )
    ...
    def _drawGrid(self):
        try:
            ...
            self._screen.blit(
                self._fontLrg.render(
                    "Turn: {}, ({})".format(
                        self._map.turn,
                        ("%.2f" % self._map.ekBot.viewInfo.lastMoveDuration).lstrip(
                            "0"
                        ),
                    ),
                    True,
                ...
                (10, self._window_size[1] - INFO_ROW_HEIGHT + 4),
            )
            self._screen.blit(
                self._font.render(self._map.ekBot.viewInfo.infoText, True, WHITE),
                (170, self._window_size[1] - INFO_ROW_HEIGHT),
            )
            if self._map.ekBot.timings:
                ...
                self._screen.blit(
                    self._font.render(
                        "Timings: {} ({})   - {}{}       {}".format(
                            timings.toString(),
                            (self._map.turn + timings.offsetTurns) % timings.cycleTurns,
                            allInText,
                            self._map.ekBot.all_in_counter,
                            self._map.ekBot.viewInfo.addlTimingsLineText,
                        ),
                        True,
                        WHITE,
                    ),
                    (170, self._window_size[1] - INFO_ROW_HEIGHT + 15),
                )
            ...
            if self._map != None:
                playersByScore = sorted(
                    self._map.players, key=lambda player: player.score, reverse=True
                )  # Sort Scores
                ...
                for i, player in enumerate(playersByScore):
                    if player != None:
                        ...
                        try:
                            self._screen.blit(
                                self._font.render(userString, True, WHITE),
                        ...
                        playerSubtext = "{} on {} ({})".format(
                            player.score, player.tileCount, player.cityCount
                        )
                        if player.index != self._map.player_index:
                            playerSubtext += " [{}]".format(
                                str(
                                    int(
                                        self._map.ekBot.playerTargetScores[player.index]
                                    )
                                )
                            )
                        self._screen.blit(
                            self._font.render(playerSubtext, True, WHITE),
                            (
                                score_width * i + 3,
                                pos_top + 1 + self._font.get_height(),
                            ),
                        )
                ...
                for column in range(self._map.cols):
                    ...
                    if tile.mountain:  # Mountain
                        ...
                        if tile.isCity or tile.isGeneral:
                            colorR = (
                                colorR + KING_COLOR_OFFSET
                                if colorR <= 255 - KING_COLOR_OFFSET
                                else 255
                            )
                            colorG = (
                                colorG + KING_COLOR_OFFSET
                                if colorG <= 255 - KING_COLOR_OFFSET
                                else 255
                            )
                            colorB = (
                                colorB + KING_COLOR_OFFSET
                                if colorB <= 255 - KING_COLOR_OFFSET
                                else 255
                            )
                        if not tile.visible:
                    ...
                    if tile in self._map.generals:  # General
                        ...
                        pygame.draw.circle(
                            self._screen,
                            color,
                            [pos_left_circle, pos_top_circle],
                            int(CELL_WIDTH / 2),
                        )
                    else:
                        # Draw Rect
                        pygame.draw.rect(
                            self._screen,
                            color,
                            [pos_left, pos_top, CELL_WIDTH, CELL_HEIGHT],
                        )

                    # mark tile territories
                    territoryMarkerSize = 5
                    tileTerritoryPlayer = self._map.ekBot.territories.territoryMap[
                        tile.x
                    ][tile.y]
                    if tile.player != tileTerritoryPlayer:
                        ...
                        if tileTerritoryPlayer != -1:
                            ...
                            pygame.draw.rect(
                                ...
                                territoryColor,
                                [
                                    pos_left + CELL_WIDTH - territoryMarkerSize,
                                    pos_top,
                                    territoryMarkerSize,
                                    territoryMarkerSize,
                                ],
            ...
            # self.draw_chokes()
            if (
                self._map.ekBot.board_analysis
                and self._map.ekBot.board_analysis.intergeneral_analysis
            ):
                chokeColor = (93, 0, 111)  # purple
                self.draw_army_analysis(
                    self._map.ekBot.board_analysis.intergeneral_analysis,
                    chokeColor,
                    draw_pathways=True,
                )
            ...
            if self._map.ekBot.targetingArmy != None:
                self.draw_square(
                    self._map.ekBot.targetingArmy.tile, 3, 1, 1, 1, 254, SQUARE_2
                )
            ...
            if (
                self._map.ekBot.dangerAnalyzer != None
                and self._map.ekBot.dangerAnalyzer.anyThreat
            ):

                for threat in [
                    self._map.ekBot.dangerAnalyzer.fastestVisionThreat,
                    self._map.ekBot.dangerAnalyzer.fastestThreat,
                    self._map.ekBot.dangerAnalyzer.highestThreat,
                ]:
                    if threat == None:
                        continue
                    # Draw danger path
            ...
            if (
                self._map != None
                and self._map.ekBot != None
                and self._map.ekBot.viewInfo.evaluatedGrid != None
                and len(self._map.ekBot.viewInfo.evaluatedGrid) > 0
            ):
                # print("if")
                for row in range(self._map.rows):
                    for column in range(self._map.cols):
                        # print("loop")
                        countEvaluated = int(
                            self._map.ekBot.viewInfo.evaluatedGrid[column][row]
                            + self._map.ekBot.viewInfo.lastEvaluatedGrid[column][row]
                        )
                        # print("loopVal")
                        if countEvaluated > 0:
                    ...
                    if tile.delta.toTile != None:
                        if tile.x - tile.delta.toTile.x > 0:  # left
                            ...
                            pygame.draw.polygon(
                                ...
                                GRAY_DARK,
                                [
                                    (pos_left + CELL_WIDTH / 4, pos_top),
                                    (pos_left + CELL_WIDTH / 4, pos_top + CELL_HEIGHT),
                                    (
                                        pos_left - CELL_WIDTH / 4,
                            ...
                            pygame.draw.polygon(
                                ...
                                GRAY_DARK,
                                [
                                    (pos_left + 3 * CELL_WIDTH / 4, pos_top),
                                    (
                                        pos_left + 3 * CELL_WIDTH / 4,
                                        pos_top + CELL_HEIGHT,
                                    ),
                                    (
                                        pos_left + 5 * CELL_WIDTH / 4,
                                        pos_top + CELL_HEIGHT / 2,
                                    ),
                                ],
                            )
                            ...
                            pygame.draw.polygon(
                                ...
                                GRAY_DARK,
                                [
                                    (pos_left, pos_top + CELL_HEIGHT / 4),
                                    (pos_left + CELL_WIDTH, pos_top + CELL_HEIGHT / 4),
                                    (
                                        pos_left + CELL_WIDTH / 2,
                            ...
                            pygame.draw.polygon(
                                ...
                                GRAY_DARK,
                                [
                                    (pos_left, pos_top + 3 * CELL_HEIGHT / 4),
                                    (
                                        pos_left + CELL_WIDTH,
                                        pos_top + 3 * CELL_HEIGHT / 4,
                                    ),
                                    (
                                        pos_left + CELL_WIDTH / 2,
            ...
            for row in range(self._map.rows):
                for column in range(self._map.cols):
                    ...
                    if (
                        not tile in self._map.ekBot.reachableTiles
                        and not tile.isobstacle()
                        and not tile.isCity
                        and not tile.mountain
                    ):
                        textVal = "   X"
                        self._screen.blit(
                            self._font.render(textVal, True, color_font),
                            (pos_left + 2, pos_top + CELL_HEIGHT / 4),
                        )

                    # Draw Text Value
                    if tile.army != 0 and (
                        tile.discovered or tile in self._map.ekBot.armyTracker.armies
                    ...
                    self._screen.blit(
                        self._fontSmall.render(textVal, True, color_font),
                        (pos_left, pos_top - 2),
                    )
                    ...
                    if self._map.ekBot.leafValueGrid != None:
                        ...
                        if leafVal != None:
                            ...
                            self._screen.blit(
                                self._fontSmall.render(textVal, True, color_font),
                                (
                                    pos_left + CELL_WIDTH / 3,
                                    pos_top + 2 * CELL_HEIGHT / 3,
                                ),
                            )
                    ...
                    if self._map.ekBot.viewInfo.bottomRightGridText != None:
                        ...
                        if text != None:
                            ...
                            self._screen.blit(
                                ...
                                (
                                    pos_left + 3 * CELL_WIDTH / 4,
                                    pos_top + 1.5 * CELL_HEIGHT / 3,
                                ),
                            )
                        elif self._map.ekBot.targetPlayer != -1:
                            val = self._map.ekBot.armyTracker.emergenceLocationMap[
                                self._map.ekBot.targetPlayer
                            ][column][row]
                            if val != 0:
    ...
    def draw_army(self, army, R, G, B, alphaStart):
        ...
        self.draw_square(
            tile, 1, playerR, playerG, playerB, min(255, int(alphaStart * 1.3))
        )
        ...
        pos_top = (CELL_MARGIN + CELL_HEIGHT) * tile.y + CELL_MARGIN
        self._screen.blit(
            self._font.render(army.name, True, WHITE),
            (pos_left + CELL_WIDTH - 10, pos_top),
        )
        ...
        if chokeColor:
            for choke in analysis.pathChokes:
                ...
                self.draw_square(
                    ...
                    chokeColor[2],
                    230,
                    SQUARE_SMALLER_INNER,
                )
        if draw_pathways:
            uniquePathways = set(analysis.pathways.values())
            logging.info(
                "pathways.values() {} vs uniquePathways {}".format(
                    len(analysis.pathways.values()), len(uniquePathways)
                )
            )
    ...
    def draw_chokes(self, R=93, G=0, B=111):
        if self._map.ekBot.board_analysis != None:
            if self._map.ekBot.board_analysis.intergeneral_analysis != None:
                for (
                    choke
                ) in self._map.ekBot.board_analysis.intergeneral_analysis.pathChokes:
                    # Poiple
    ...
    def draw_between_tiles(self, shape, sourceTile, destTile):
        ...
        if xDiff > 0:
            pos_left = (
                (CELL_MARGIN + CELL_WIDTH) * sourceTile.x
                + CELL_WIDTH // 2
                + CELL_MARGIN
            )
        ...
        if yDiff > 0:
            ...
            pos_top = (
                (CELL_MARGIN + CELL_HEIGHT) * sourceTile.y
                + CELL_HEIGHT // 2
                + CELL_MARGIN
            )
            s = shape.upShape
    ...
    def save_images(self):
        while True:
            if self._readyRender:
                ...
                pygame.image.save(
                    self._screen, "{}\\{}.png".format(self.logDirectory, self._map.turn)
                )
                self._readyRender = False
            time.sleep(0.1)
