...
class GeneralsViewer(object):
    def __init__(self, name=None, moveEvent=None):
        ...
        self._name = name
        self._moveEvent = (
            moveEvent  # self._moveEvent([source_x, source_y], [target_x, target_y])
        )
    ...
    def _handleClick(self, pos):
        if pos[1] < ABOVE_GRID_HEIGHT:
            if pos[0] < TOGGLE_GRID_BTN_WIDTH:  # Toggle Grid
                ...
                pos[0] < TOGGLE_GRID_BTN_WIDTH + TOGGLE_EXIT_BTN_WIDTH
            ):  # Toggle Exit on Game Over
                self._map.exit_on_game_over = not self._map.exit_on_game_over
            self._receivedUpdate = True
        elif (
            self._showGrid
            and pos[1] > ABOVE_GRID_HEIGHT
            and pos[1] < (self._window_size[1] - SCORES_ROW_HEIGHT)
        ):  # Click inside Grid
    ...
    def _drawActionbar(self):
        ...
        pygame.draw.rect(
            self._screen,
            (0, 80, 0),
            [0, 0, TOGGLE_GRID_BTN_WIDTH, ACTIONBAR_ROW_HEIGHT],
        )
        ...
        pygame.draw.rect(
            self._screen,
            (0, 100, 0) if self._map.exit_on_game_over else (90, 0, 0),
            [TOGGLE_GRID_BTN_WIDTH, 0, TOGGLE_EXIT_BTN_WIDTH, ACTIONBAR_ROW_HEIGHT],
        )
        self._screen.blit(
            self._font.render("Auto Quit", True, WHITE), (TOGGLE_GRID_BTN_WIDTH + 10, 5)
        )

        # Info Text
        self._screen.blit(
            self._fontLrg.render("Turn: %d" % self._map.turn, True, WHITE),
            (self._window_size[0] - 200, 5),
        )
        self._screen.blit(
            self._font.render("%s" % self._bottomText, True, WHITE),
            (self._window_size[0] - 90, 12),
        )
    ...
    def _drawScores(self):
        ...
        for i, score in enumerate(self._scores):
            ...
            self._screen.blit(
                self._font.render(self._map.usernames[int(score["i"])], True, WHITE),
                (score_width * i + 3, pos_top + 1),
            )
            self._screen.blit(
                self._font.render(
                    str(score["total"]) + " on " + str(score["tiles"]), True, WHITE
                ),
    ...
    def _drawGrid(self):
        for row in range(self._map.rows):
            for column in range(self._map.cols):
                ...
                pos_top = (
                    (CELL_MARGIN + CELL_HEIGHT) * row + CELL_MARGIN + ABOVE_GRID_HEIGHT
                )
                ...
                if tile.isSwamp:
                    self._screen.blit(
                        self._font.render("±", True, color_font),
                        (pos_left + 9, pos_top + 7),
                    )

                # Draw Path
                if self._path != None and (column, row) in self._path:
                    self._screen.blit(
                        self._fontLrg.render("*", True, color_font),
                        (pos_left + 5, pos_top - 3),
                    )
                if self._collect_path != None and (column, row) in self._collect_path:
                    self._screen.blit(
                        self._fontLrg.render("*", True, PLAYER_COLORS[8]),
                        (pos_left + 6, pos_top + 6),
                    )
