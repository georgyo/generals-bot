...
class Map(object):
    def __init__(self, start_data, data):
        ...
        self.replay_url = (
            REPLAY_URLS["na"] + start_data["replay_id"]
        ...
        self.grid = [
            [Tile(self, x, y) for x in range(self.cols)] for y in range(self.rows)
        ]  # 2D List of Tile Objects
        self._setNeighbors()
        self.swamps = [
            (c // self.cols, c % self.cols) for c in start_data["swamps"]
        ]  # List [(y,x)] of swamps
        self._setSwamps()
        ...
        ]  # List of 8 (+ extra) Generals (None if not found)
        self._setGenerals()
    ...
    def find_largest_tile(
        ...
        if general == None:
            logging.error(
                "ERROR: find_largest_tile encountered general=None for player %d with list %s"
        ...
        for tile in self.tiles[ofType]:  # Check each ofType tile
            if largest == None or largest.army < tile.army:  # New largest
                if (
                    not tile.isGeneral and tile not in notInPath
                ):  # Exclude general and path
                    largest = tile

        if (
            includeGeneral > 0 and general != None and general not in notInPath
        ):  # Handle includeGeneral
            if includeGeneral < 1:
    ...
    def find_primary_target(self, target=None):
        ...
        for x in _shuffle(range(self.cols)):  # Check Each Tile
            for y in _shuffle(range(self.rows)):
                ...
                if (
                    not source.isValidTarget() or source.tile == self.player_index
    ...
    def isValidPosition(self, x, y):
        return (
            0 <= y < self.rows
            and 0 <= x < self.cols
            and self._tile_grid[y][x] != TILE_MOUNTAIN
        )
