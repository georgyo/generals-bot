...
class BotCommands(object):
    ...
    def _get_command(self, msg, isFromChat, username):
        ...
        if (
            command_len > 1 and "_map" in dir(self) and "usernames" in dir(self._map)
    ...
    def _handleRestrictedCommand(self, commandlist):
        ...
        if (
            self._permitted_username != "" and self._permitted_username != username
        ):  # Only allow permitted user
            return False
    ...
    def _add_teammate(self, username):
        if "_map" in dir(self) and "usernames" in dir(self._map):
            if (
                username != ""
                and username != self._map.usernames[self._map.player_index]
                and username in self._map.usernames
            ):
                self._map.do_not_attack_players.append(
                    self._map.usernames.index(username)
                )
                return True
        return False
    ...
    def _remove_teammate(self, username):
        if "_map" in dir(self) and "usernames" in dir(self._map):
            if (
                ...
                if (
                    self._map.usernames.index(username)
                    in self._map.do_not_attack_players
                ):
                    self._map.do_not_attack_players.remove(
                        self._map.usernames.index(username)
                    )
                    return True
        return False
