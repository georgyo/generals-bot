...
class Generals(object):
    def __init__(
        ...
        self._connect_and_join(
            userid, username, mode, gameid, force_start, public_server
        )
    ...
    def send_chat(self, msg):
        ...
        try:
            if self._seen_update:
                self._send(
                    ["chat_message", self._start_data["chat_room"], msg, None, ""]
                )
            elif self._gameid != None:
                self._send(
                    ["chat_message", "chat_custom_queue_" + self._gameid, msg, None, ""]
                )
        except WebSocketConnectionClosedException:
            pass
    ...
    def _log_queue_update(self, msg):
        if "queueTimeLeft" in msg:
            logging.info(
                "Queue (%ds) %d/%d"
                % (msg["queueTimeLeft"], msg["numForce"], msg["numPlayers"])
            )
            return
        ...
        if "map_title" in msg:
            ...
            if mapname and len(mapname) > 1:
                logging.info(
                    "Queue [%s] %d/%d %s"
                    % (mapname, msg["numForce"], msg["numPlayers"], self.teams)
                )
                return

        logging.info(
            "Queue %d/%d %s" % (msg["numForce"], msg["numPlayers"], self.teams)
        )
    ...
    def _make_update(self, data):
        if not self._seen_update:
            ...
            logging.info(
                "Joined Game: %s - %s" % (self._map.replay_url, self._map.usernames)
            )
            return self._map
    ...
    def _handle_chat(self, chat_msg):
        if "username" in chat_msg:
            self.handle_command(
                chat_msg["text"], from_chat=True, username=chat_msg["username"]
            )
    ...
    def _connect_and_join(
        self, userid, username, mode, gameid, force_start, public_server
    ):
        logging.debug("Creating connection")
        self._ws = create_connection(
            ENDPOINT_BOT if not public_server else ENDPOINT_PUBLIC
        )
    ...
    def set_normal_map(self, width=-1, height=-1, city=-1, mountain=-1, swamp=-1):
        ...
        if mountain >= 0 and mountain <= 1:
            self._send(
                ["set_custom_options", self._gameid, {"mountain_density": mountain}]
            )
