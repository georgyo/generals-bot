...
class Tile(object):
    ...
    def update(self, gamemap, tile, army, isCity=False, isGeneral=False, isDirty=False):
        ...
        if (
            self.tile < 0
            or tile >= TILE_MOUNTAIN
            or (tile < TILE_MOUNTAIN and self.isSelf())
        ):  # Tile should be updated
            if (
                tile >= 0 or self.tile >= 0
            ) and self.tile != tile:  # Remember Discovered Tiles
            ...
            self.tile = tile
        if (
            self.army == 0 or army > 0 or tile >= TILE_MOUNTAIN or self.isSwamp
    ...
    def neighbors(self, includeSwamps=False, includeCities=True):
        ...
        for tile in self._neighbors:
            if (
                (tile.tile != TILE_OBSTACLE or tile.isCity or tile.isGeneral)
                and tile.tile != TILE_MOUNTAIN
                and (includeSwamps or not tile.isSwamp)
                and (includeCities or not tile.isCity)
            ):
    ...
    def neighbor_to_attack(self, path=[]):
        ...
        for neighbor in self.neighbors(includeSwamps=True):
            if (
                neighbor.shouldAttack() and self.army > neighbor.army + 1
            ) or neighbor in path:  # Move into caputurable target Tiles
                if not neighbor.isSwamp:
                    if target == None:
                        target = neighbor
                    elif neighbor.isCity and (
                        not target.isCity or target.army > neighbor.army
                    ):
                        target = neighbor
                    elif (
                        not neighbor.isEmpty and neighbor.army <= 1 and target.isEmpty
                    ...
                    neighbor.turn_held == 0
                ):  # Move into swamps that we have never held before
                    target = neighbor
    ...
    def nearest_target_tile(self):
        ...
        for x in range(self._map.cols):  # Check Each Square
            for y in range(self._map.rows):
                ...
                if (
                    not tile.isValidTarget()
                    or tile.shouldNotAttack()
                    or tile.army > max_target_army
                ):  # Non Target Tiles
                    continue
                ...
                if tile.isGeneral:  # Generals appear closer
                    ...
                    distance = (
                        distance
                        * sorted((0.17, (tile.army / (3.2 * self.army)), 20))[1]
                    )
                ...
                if tile.isSwamp:  # Swamps appear further away
                    ...
                    if (
                        tile.turn_held > 0
                    ):  # Swamps which have been held appear even further away
                        distance = distance * 3
    ...
    def path_to(self, dest, includeCities=False):
        ...
        while not frontier.empty():
            ...
            for next in current.neighbors(
                includeSwamps=True, includeCities=includeCities
            ):
                if next not in came_from and (
                    next.isOnTeam() or next == dest or next.army < army_count[current]
                ):
