...
def startup(moveMethod, botName="PurdueBot"):
    ...
    parser.add_argument(
        ...
        metavar="str",
        type=str,
        default=os.environ.get("GENERALS_BOT_NAME", botName),
        help="Name of Bot",
    )
    parser.add_argument(
        "-g",
        ...
        default="ffa",
        help="Game Type: private, custom, 1v1, ffa, or team",
    )
    # parser.add_argument('--roomID', metavar='str', type=str, default="EklipZ_ai", help='Private Room ID (optional)')
    parser.add_argument(
        "--roomID", metavar="str", type=str, help="Private Room ID (optional)"
    )
    parser.add_argument("--right", action="store_true")
    parser.add_argument("--bottom", action="store_true")
    parser.add_argument(
        "--no-ui", action="store_false", help="Hide UI (no game viewer)"
    )
    parser.add_argument(
        "--public", action="store_true", help="Run on public (not bot) server"
    )
    ...
    bot_base.GeneralsBot(
        ...
        gameViewer=args["no_ui"],
        public_server=args["public"],
        alignBottom=args["bottom"],
        alignRight=args["right"],
    )
