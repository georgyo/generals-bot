...
class PathColorer(object):
    def __init__(
        self, path, r, g, b, alpha=255, alphaDecreaseRate=10, alphaMinimum=100
    ):
        self.path = path
        self.color = (r, g, b)
        self.alpha = alpha
...
class ViewInfo(object):
    def __init__(self, countHist, cols, rows):
        ...
        self.bottomRightGridText = [
            [None for x in range(self.rows)] for y in range(self.cols)
        ]
        self.bottomLeftGridText = [
            [None for x in range(self.rows)] for y in range(self.cols)
        ]
    ...
    def turnInc(self):
        ...
        for i in range(countHist):
            if i == countHist - 2:
                break
            self.searchHistory[countHist - i - 1] = self.searchHistory[
                countHist - i - 2
            ]
        ...
        if len(self.lastEvaluatedGrid) == 0:
            self.lastEvaluatedGrid = [
                [0 for x in range(self.rows)] for y in range(self.cols)
            ]
